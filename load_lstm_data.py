...
def split_test_train(data_dict, year):
    obs_drop = ['year','day','hour', 'x1', 'x2', 'p']
    data_dict['obs_test'] = data_dict['obs'].loc[data_dict['obs']['year'] >= year-1]
    data_dict['obs_train'] = data_dict['obs'].loc[data_dict['obs']['year'] < year-1]
    ...
    data_dict['forcing_test'] = data_dict['forcing'].loc[data_dict['obs']['year'] >= year-1]
    data_dict['forcing_train'] = data_dict['forcing'].loc[data_dict['obs']['year'] < year-1]
...
def load_lstm_data(data_dir, site, year):
    ...
    data_dict = split_test_train(data_dict, 2006)
    return data_dict
